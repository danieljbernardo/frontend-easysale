...
formPesquisa.addEventListener("submit", async (event) => {
  ...
  if (!validarCPF(pesquisaCpf.value.trim())) return alert("CPF inválido.");

  const body = { cpf: pesquisaCpf.value.trim(), data: pesquisaData.value };
  ...
  try {
    ...
    if (response.ok) {
      ...
      vendasEncontradas = Array.isArray(resultado) ? resultado : (resultado ? [resultado] : []);
      if (vendasEncontradas.length === 0) return alert("Nenhuma venda encontrada.");
      preencherVendas(vendasEncontradas);
    } else {
      alert(await response.text());
    }
  } catch (error) { console.error(error); alert("Erro ao buscar venda."); }
});
...
function preencherVendas(vendas) {
  resultadoVenda.style.display = "block";
  let html = `<h6 style="margin:15px 0 10px;">Vendas encontradas: ${vendas.length}</h6>`;

  vendas.forEach(venda => {
    const vendaId = venda.vendaId || venda.id;
    const nf = venda.notaFiscalNumero || (venda.notaFiscal ? venda.notaFiscal.numeroNota : "—");
    const total = calcularTotalVenda(venda.itensVenda || []);

    html += `
      <div style="border:1px solid #dee2e6; border-radius:8px; padding:15px; margin-bottom:20px; background:#f8f9fa;">
        <div class="row mb-3">
          <div class="col-md-3"><strong>CPF:</strong> ${formatarCPF(venda.clienteCpf)}</div>
          <div class="col-md-2"><strong>Nota Fiscal:</strong> ${nf}</div>
          <div class="col-md-2"><strong>Data:</strong> ${formatarDataExibicao(venda.dataVenda)}</div>
          <div class="col-md-2"><strong>Pagamento:</strong> ${venda.pagamentoForma}</div>
          <div class="col-md-3"><strong>Total:</strong> R$ ${total.toFixed(2).replace(".", ",")}</div>
        </div>
        Itens da Venda
        ${renderizarItens(venda.itensVenda || [], vendaId)}
      </div>`;
  });
...
function renderizarItens(itens, _vendaId) {
  if (!itens || itens.length === 0) return `<p class="text-muted">Nenhum item nesta venda.</p>`;

  let html = `<table class="tabela-itens"><thead><tr><th>Cód. Produto</th><th>Nome</th><th>Qtd.</th><th>Preço Unit.</th><th>Subtotal</th><th>Ações</th></tr></thead><tbody>`;

  itens.forEach(item => {
    const itemId = item.itemVendaId || item.id;
    html += `
      <tr id="linha-item-${itemId}">
        <td><span class="item-display-codigo">${item.produtoCodigo}</span><input type="number" class="form-control form-control-sm item-edit-codigo" value="${item.produtoCodigo}" style="display:none;width:90px;"></td>
        <td>${item.produtoNome}</td>
        <td><span class="item-display-qtd">${item.quantidade}</span><input type="number" class="form-control form-control-sm item-edit-qtd" value="${item.quantidade}" style="display:none;width:80px;"></td>
  ...
  document.querySelectorAll(".btn-editar-item").forEach(btn => btn.addEventListener("click", () => entrarEdicaoItem(btn.dataset.id)));
  document.querySelectorAll(".btn-salvar-item").forEach(btn => btn.addEventListener("click", () => salvarEdicaoItem(btn.dataset.id)));
  document.querySelectorAll(".btn-excluir-item").forEach(btn => btn.addEventListener("click", () => excluirItem(btn.dataset.id)));
}
...
async function salvarEdicaoItem(id) {
  ...
  if (!validarCodigo(codigo)) return alert("Código inválido (100-999).");
  if (isNaN(qtd) || qtd < 1) return alert("Quantidade inválida.");

  const body = { itemVendaId: parseInt(id), produtoCodigo: codigo, quantidade: qtd };
...
cancelarNovoItemBtn.addEventListener("click", () => formNovoItemWrapper.style.display = "none");
...
salvarNovoItemBtn.addEventListener("click", async () => {
  ...
  if (!validarCodigo(codigo)) return alert("Código inválido.");
  if (isNaN(qtd) || qtd < 1) return alert("Quantidade inválida.");
  if (isNaN(preco) || preco <= 0) return alert("Preço inválido.");

  const body = {
    vendaId: vendasEncontradas[0].vendaId || vendasEncontradas[0].id,
...
async function rebuscarVenda() {
  const body = { cpf: pesquisaCpf.value.trim(), data: pesquisaData.value };
  try {
    const reponse = await fetch("/easysale/venda/buscar-venda", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
      body: JSON.stringify(body)
    });
    if (reponse.ok) {
      const resultado = await reponse.json();
      vendasEncontradas = Array.isArray(resultado) ? resultado : [resultado];
      preencherVendas(vendasEncontradas);
    }
  } catch (error) { console.error(error); }
}

adicionarItemCadastroBtn.addEventListener("click", () => formItemCadastroWrapper.style.display = "block");
cancelarItemCadastroBtn.addEventListener("click", () => formItemCadastroWrapper.style.display = "none");
...
function renderizarItensCadastro() {
  ...
  itensCadastro.forEach((item, index) => {
    ...
    div.innerHTML = `
      ...
      <button type="button" class="btn botao-excluir-sm" data-index="${index}"><i class="bi bi-trash"></i></button>`;
...
formCadastrarVenda.addEventListener("submit", async (event) => {
  event.preventDefault();
  if (!validarCPF(cpfCadastro.value.trim())) return alert("CPF inválido.");
  if (!pagamentoCadastro.value) return alert("Selecione a forma de pagamento.");
  if (itensCadastro.length === 0) return alert("Adicione ao menos um item.");

  const total = itensCadastro.reduce((acc, i) => acc + i.subtotal, 0);

  const body = {
    clienteCpf: cpfCadastro.value.trim(),
    pagamentoForma: pagamentoCadastro.value,
    valorTotal: parseFloat(total.toFixed(2)),
    itensVenda: itensCadastro.map(i => ({
  ...
  try {
    const reponse = await fetch("/easysale/venda/cadastrar-venda", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
      body: JSON.stringify(body)
    });
...
cancelarEdicaoBtn.addEventListener("click", () => {
  if (vendasEncontradas.length > 0) preencherVendas(vendasEncontradas);
...
formVenda.addEventListener("submit", async (event) => {
  ...
  const body = {
    ...
    valorTotal: calcularTotalVenda(vendasEncontradas[0]?.itensVenda || []),
...
async function excluirVenda(vendaId) {
  ...
  try {
    const res = await fetch(`/easysale/venda/${vendaId}`, { method: "DELETE", headers: { Authorization: "Bearer " + token } });
    if (res.ok) {
...
gerarRelatorioBtn.addEventListener("click", async () => {
  if (!vendasEncontradas.length) return alert("Faça uma pesquisa primeiro.");
  try {
    const body = { cpf: pesquisaCpf.value.trim(), data: pesquisaData.value };
    const response = await fetch("/easysale/venda/relatorio", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
      body: JSON.stringify(body)
    });
    if (response.ok) {
      ...
      a.href = url; a.download = "relatorio_vendas.pdf"; a.click();
